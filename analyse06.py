...
data = data.loc[data['time_saved']>="2018-11-25 18:51:03.401308"]
data = data.loc[data['time_saved']<="2018-11-27 18:51:03.401308"]
...
def identifie_bus(data):
    ...
    while len(data2)!=0:
        ...
        if len(cur>2):
            while cur.iloc[0]['minutes']>0 or cur.iloc[1]['minutes']-cur.iloc[0]['minutes']<1\
            or  (cur.iloc[1]['time_saved']-cur.iloc[0]['time_saved']).seconds<60*cur.iloc[0]['minutes']:
                ...
                if len(cur) == 1 or (cur.iloc[0]['minutes'] == 0 and cur.iloc[1]['minutes']-cur.iloc[0]['minutes']>1) \
